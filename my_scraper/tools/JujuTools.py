...
class Parser:
    ...
    def buffaloschools_applicantstack_com(self, soup):
        # list(soup.find("div", id="ascontainer").div.next_siblings)[1].get_text(strip=True)
    ...
    def recruit_zohopublic_com(self, soup):
        # desc = soup.find("div", attrs={"class": "job-dec-block_new"})
        # desc = desc.get_text(strip=True)
        return "js site"
    ...
    def anyonehiring_com(self, soup):
        # list(soup.find("div", attrs={"class": "job-leftcontent"}).next_siblings)[1].get_text(strip=True)
...
def get_juju_description(response):
    ...
    try:
        ...
        raise scrapy.exceptions.CloseSpider("No parser for: [{}], netloc: [{}]".format(response.url, netloc))
...
broken_links = [
    ...
    'http://www.prodivnet.com/job-summaries?job=5799e554e796554880000348&organization=Chicago+Public+Schools',
    'http://www.prodivnet.com/job-summaries?job=57a07b6ee79655a32100156c&organization=Chicago+Public+Schools',
    'http://www.prodivnet.com/job-summaries?job=57934e59e79655741200133b&organization=Chicago+Public+Schools',
    'http://www.prodivnet.com/job-summaries?job=5799e624e79655488000038c&organization=Chicago+Public+Schools',
    'http://www.prodivnet.com/job-summaries?job=57a0760fe79655b30a0001ef&organization%20=Seattle+Public+Schools',
    'http://www.prodivnet.com/job-summaries?job=57a0760fe79655b30a0001ef&organization=Seattle+Public+Schools',

    'http://jobsearch.educationamerica.net/index.phtml?a=v&',
    'http://findjobinfo.com',

    'https://simplyjobs.com/applicants/register/sign-up?feed=DirectEmployers&job_id=iCIMS14507-5975',
    'https://simplyjobs.com/applicants/register/sign-up?feed=DirectEmployers&job_id=iCIMS14507-5877'
]
